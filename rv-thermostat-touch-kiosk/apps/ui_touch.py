...
class TouchUI(tk.Tk):
    def __init__(self, fullscreen=True, hide_cursor=True):
        ...
        if hide_cursor:
            # OPTION A: hide mouse cursor inside our window
    ...
    def _build_layout(self):
        ...
        # Top bar with title, zone badge, Wi-Fi indicator
        ...
        main = tk.Frame(self, bg=COL_BG); main.grid(row=1, column=0, sticky="nsew", padx=16, pady=(10,16))
        main.grid_columnconfigure(0, weight=1); main.grid_rowconfigure(0, weight=1)

        card = tk.Frame(main, bg=COL_PANEL); card.grid(row=0, column=0, sticky="nsew")
        ...
        left = tk.Frame(card, bg=COL_PANEL); left.grid(row=0, column=0, rowspan=6, sticky="nsw", padx=(14,0), pady=14)
        tk.Label(left, text="Outside", fg=COL_AMBER, bg=COL_PANEL, font=self.f_caps).pack(anchor="w")
        self.outside_var = tk.StringVar(value="-- °F")
        ...
        self.ind_cool = tk.Label(ind, text="❄ COOL", fg=COL_MUTED, bg=COL_PANEL, font=self.f_small); self.ind_cool.grid(row=0, column=0, padx=(0,14))
        self.ind_heat = tk.Label(ind, text="🔥 HEAT", fg=COL_MUTED, bg=COL_PANEL, font=self.f_small); self.ind_heat.grid(row=0, column=1, padx=(0,14))
        self.ind_fan  = tk.Label(ind, text="🌀 FAN",  fg=COL_MUTED, bg=COL_PANEL, font=self.f_small); self.ind_fan.grid(row=0, column=2)

        # Center: big °F, state text
        center = tk.Frame(card, bg=COL_PANEL); center.grid(row=0, column=1, rowspan=4, sticky="nsew")
        center.grid_columnconfigure(0, weight=1); center.grid_rowconfigure(0, weight=1)
        self.lbl_bigtemp = tk.Label(center, text="--", fg=COL_CYAN, bg=COL_PANEL, font=self.f_big); self.lbl_bigtemp.grid(row=0, column=0, sticky="n", pady=(10,0))
        self.lbl_state   = tk.Label(center, textvariable=self.state_var, fg=COL_MUTED, bg=COL_PANEL, font=self.f_small); self.lbl_state.grid(row=1, column=0, pady=(0,8))

        # Right action bar (placeholders)
        right = tk.Frame(card, bg=COL_PANEL); right.grid(row=0, column=2, rowspan=6, sticky="nse", padx=(0,14), pady=14)
        ...
        # Mode selectors
        modes = tk.Frame(card, bg=COL_PANEL); modes.grid(row=4, column=1, sticky="n", pady=(0,6))
        ...
        # Setpoint nudgers
        nudges = tk.Frame(card, bg=COL_PANEL); nudges.grid(row=4, column=0, sticky="w", padx=12)
        ...
        self.lbl_setpoint = tk.Label(nudges, text=self._format_sp_f(), fg=COL_TEXT, bg=COL_PANEL, font=self.f_mid); self.lbl_setpoint.pack(side="left", padx=8)
        tk.Button(nudges, text="+", fg=COL_TEXT, bg=COL_GRAYBTN, bd=0, relief="flat",
                  font=self.f_mid, width=2, command=lambda: self.bump_setpoint(+0.5)).pack(side="left", padx=4)

        # Bottom action buttons
        bottom = tk.Frame(card, bg=COL_PANEL); bottom.grid(row=5, column=0, columnspan=3, sticky="ew", pady=(0,12), padx=12)
        bottom.grid_columnconfigure(0, weight=1); bottom.grid_columnconfigure(1, weight=1)
        self.btn_cool = tk.Button(bottom, text="Cool to 76°F", command=self.cool_to_target,
        ...
        self.btn_heat = tk.Button(bottom, text="Heat to 68°F", command=self.heat_to_target,
                                  fg=COL_TEXT, bg=COL_RED, activebackground="#d63c3c",
                                  activeforeground=COL_TEXT, bd=0, relief="flat", font=self.f_mid, height=1)
        self.btn_heat.grid(row=0, column=1, sticky="nsew", padx=(8,0))

    # ---------------- Behavior ----------------
    def _format_sp_f(self) -> str: return f"{round(c_to_f(self.setpoint_var.get())):.0f}°F"

    def bump_setpoint(self, delta: float):
        val = round((self.setpoint_var.get() + delta) * 2) / 2.0
        self.setpoint_var.set(val); self.cfg.control.setpoint_c = float(val)
    ...
    def cool_to_target(self):
        self.mode_var.set("cool"); self.cfg.control.mode = "cool"
    ...
    def heat_to_target(self):
        self.mode_var.set("heat"); self.cfg.control.mode = "heat"
    ...
    def refresh_readings(self):
        ...
        self.outside_var.set("-- °F")  # placeholder
