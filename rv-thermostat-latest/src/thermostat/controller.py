...
class ThermostatController:
    ...
    def _start_cooling(self):
        if not self._can_start_compressor():
            self.log("[CTRL] Cooling demand but waiting for min-off-time…"); return
        self.log("[CTRL] Cooling ON"); self.act.cool_on()
        self.s.current_mode = "cooling"; self.s.last_cool_on_at = time.time()
