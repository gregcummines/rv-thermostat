    ...
    try:
        ...
        for line in p.stdout.splitlines():
            ...
            if line.startswith('BSS '):
                if mac:
                    aps.append({'macAddress':mac,'signalStrength': int(sig) if sig is not None else -50})
...
def _wifi_loc_mls(iface:str, api_key:str|None)->Optional[GeoResult]:
    ...
    try:
        ...
        if r.ok:
            j=r.json(); return {'lat':j['location']['lat'],'lon':j['location']['lng'],'source':'wifi-mls','accuracy_m':float(j.get('accuracy',100.0))}
...
def _ip_loc(provider:str='ip-api')->Optional[GeoResult]:
    ...
    try:
        if provider=='ipinfo':
            ...
            if r.ok:
                j=r.json(); lat,lon=map(float,j['loc'].split(',')); return {'lat':lat,'lon':lon,'source':'ipinfo','accuracy_m':50000.0}
        else:
            r=requests.get('http://ip-api.com/json', timeout=4)
            if r.ok and r.json().get('status')=='success':
                j=r.json(); return {'lat':float(j['lat']),'lon':float(j['lon']),'source':'ip-api','accuracy_m':50000.0}
    except Exception: pass
    return None
