...
class RelayOut:
    def __init__(self,pin:int,active_low:bool=True,name:str='relay'):
        ...
        GPIO.setup(self.pin, GPIO.OUT, initial=self._level(False)); self.state=False
    def _level(self,on:bool):
        return GPIO.LOW if (on and self.active_low) else (GPIO.HIGH if on else (GPIO.HIGH if self.active_low else GPIO.LOW))
